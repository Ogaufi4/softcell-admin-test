...
import { dealerVirtualStock } from "@/data/dealer";
...
export default function DealerVirtualStock() {
  const t = useTranslations("dealer.virtualStock");

  const statusOf = (item: (typeof dealerVirtualStock)[number]) => {
  ...
  const totalCredit = dealerVirtualStock.reduce((s, i) => s + i.credit, 0);
  const totalAvailable = dealerVirtualStock.reduce(
    (s, i) => s + i.available,
    0,
  );
